...
class TestImportMap:
    def test_importmap_basic(self):
        ...
        assert (
            import_map["https://www.example.org/regular"] == "s3://this.file"
        )
        # except when a concrete exception has been added to the ImportMap
        assert (
            import_map["https://www.example.org/exception"]
            == "s3://another.file"
        )
        ...
        assert set(import_map.values()) == set(
            ["s3://another.file", "s3://this.file"]
        )
...
class TestConfigMap:
    def test_import_config(self, monkeypatch):
        ...
        connection_map = {
            "my_s3_bucket": S3Config(env_prefix="TEST_"),
            "my_file_system": LocalFileConfig(),
        }
        import_map = {
